...
// Compile command:  gcc -Wall LJ8in3D_bicubicFE_binning.c MargotColVar_CoordNum.c timestep_helpers.c -lm -O3
...
void total_pot_and_grad(double *conf,int Natoms,double *pot,double *grad,
	double *CVval,double *CVgrad,struct NN *nncv,
...
void binning_trajectory(long *bins,double *grid_CV1,double *grid_CV2,
	...
	double *bicubic_matrix,double *bin_confs,struct NN *nncv);
...
void binning_trajectory(long *bins,double *grid_CV1,double *grid_CV2,
	...
	int j,n,k,dim = Natoms*DIM,j1,j2,kshift,dim4 = nncv->dim4;
	int ind;
	double *gauss01;
	...
	CVgrad = (double *)malloc(dim*dim4*sizeof(double));
	...
	total_pot_and_grad(conf0,Natoms,Vpot0,Vgrad0,CVval,CVgrad,nncv,
	...
	// evaluate the potential and the gradient at the proposed point
		total_pot_and_grad(conf1,Natoms,Vpot1,Vgrad1,CVval,CVgrad,nncv,
			grid_CV1,grid_CV2,h1,h2,bicubic_matrix,FEpot,FEder1,FEder2);
...
// Compute coefficient matrices for the bicubic spline	
	...
	printf("Created the initial configuration\n");
 	...
 	binning_trajectory(bins,grid_CV1,grid_CV2,Nsteps,Natoms,
		conf0,dt,h1,h2,bicubic_matrix,bin_confs,nncv);
