...
void MargotCV(double *cvval,double *conf,int Natoms,struct NN *nncv){
	...
	int dim3 = nncv->dim3, dim4 = nncv->dim4;
	double *A1 = nncv->A1,*b1 = nncv->b1,*A2 = nncv->A2,*b2 = nncv->b2;
	double *A3 = nncv->A3,*b3 = nncv->b3, *A4 = nncv->A4,*b4 = nncv->b4;
	...
	Axpy(A3,w2,b3,w3,dim2,dim3);
	for( j = 0; j < dim3; j++ ) {
		w3[j] = ELU(w3[j]);
	}
	Axpy(A4,w3,b4,cvval,dim3,dim4);
...
void MargotCVgrad(double *grad,double *cvval,double *conf,int Natoms,
	...
	double *D1,*D2,*D3,*Jaux1,*Jaux2,*Jaux3,*Jsort;
	double *d2,*w1,*w2,*w3;
	...
	Jaux3 = (double *)malloc(dim3*dim*sizeof(double));	
	...
	// compute D(w2)/Dxy = D3*A3*Jaux2
	matmul(A3,Jaux2,Jaux3,dim3,dim2,dim); // A3*Jaux2, size dim3-by-dim
	matmul_diag_times_full(D3,Jaux3,dim3,dim); 
	
	
	matmul(A4,Jaux3,grad,dim4,dim3,dim); // A4*Jaux3, size dim3-by-dim
	Axpy(A4,w3,b4,cvval,dim3,dim4);
...
void readCVdata(char *fname_par,char *fname_dim,struct NN *nncv) {
	...
	fscanf(f_dim,"%i\t",&dim4);
	fclose(f_dim);
	printf("NN dimensions: %i -> %i -> %i -> %i -> %i\n",dim0,dim1,dim2,dim3,dim4);
