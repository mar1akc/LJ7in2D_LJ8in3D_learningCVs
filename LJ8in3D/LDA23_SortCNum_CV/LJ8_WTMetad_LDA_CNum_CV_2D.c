...
// Compile command:  gcc -Wall LJ8_WTMetad_LDA_CNum_CV_2D.c LDA_CoordNum_CV.c timestep_helpers.c -lm -O3
...
void GaussBumps_CV_pot_and_grad(double *conf,int Natoms,double *val1,double *val2,
...
void total_pot_and_grad(double *conf,int Natoms,double *val1,double *val2,
	...
	GaussBumps_CV_pot_and_grad(conf,Natoms,val1,val2,CVval,CVgrad,height,
			sig2,Nbumps,pot,grad,biasing_pot,CVdim,LDAmatrix);	
...
void WTMetadynamics(int Nsteps_between_deposits,int Nbumps_max,int Natoms,
	...
	total_pot_and_grad(conf0,Natoms,val1,val2,CVval,CVgrad,height,
			sig2,Nbumps,Vpot0,Vgrad0,biasing_pot0,CVdim,LDAmatrix);
	...
	for( Nbumps = 0; Nbumps<Nbumps_max; Nbumps++ ) {	
		...
		for( n=0; n < Nsteps_between_deposits; n++ ) {
			...
			total_pot_and_grad(conf1,Natoms,val1,val2,CVval,CVgrad,height,
					sig2,Nbumps,Vpot1,Vgrad1,biasing_pot1,CVdim,LDAmatrix);
...
int main(void){
	...
	WTMetadynamics(Nsteps_between_deposits,Nbumps_max,Natoms,
		conf0,dt,val1,val2,height,CVdim,LDAmatrix);
