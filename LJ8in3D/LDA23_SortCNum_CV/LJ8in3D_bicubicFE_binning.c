...
// Compile command:  gcc -Wall LJ8in3D_bicubicFE_binning.c LDA_CoordNum_CV.c timestep_helpers.c -lm -O3
...
void total_pot_and_grad(double *conf,int Natoms,double *pot,double *grad,
	double *CVval,double *CVgrad,int CVdim,double *LDAmatrix,
...
void binning_trajectory(long *bins,double *grid_CV1,double *grid_CV2,
	...
	double *bicubic_matrix,double *bin_confs,int CVdim,double *LDAmatrix);
...
void total_pot_and_grad(double *conf,int Natoms,double *pot,double *grad,
	...
	evaluate_Gpot_and_ders(grid_CV1,grid_CV2,h1,h2,bicubic_matrix,CVval[0],CVval[1],
...
void binning_trajectory(long *bins,double *grid_CV1,double *grid_CV2,
	...
	double *CVval,*CVgrad,*FEpot,*FEder1,*FEder2;
	char ch;
	...
	CVgrad = (double *)malloc(dim*CVdim*sizeof(double));
	...
	total_pot_and_grad(conf0,Natoms,Vpot0,Vgrad0,CVval,CVgrad,CVdim,LDAmatrix,
	...
	// evaluate the potential and the gradient at the proposed point
		total_pot_and_grad(conf1,Natoms,Vpot1,Vgrad1,CVval,CVgrad,CVdim,LDAmatrix,
			grid_CV1,grid_CV2,h1,h2,bicubic_matrix,FEpot,FEder1,FEder2);
		...
		j1 = min(max(0,(int)floor((CVval[0] - grid_CV1[0])/h1+0.5)),N1-1);
		j2 = min(max(0,(int)floor((CVval[1] - grid_CV2[0])/h2 +0.5)),N2-1);
		ind = j1 + j2*N1;
// 		printf("%i, %i\n",j1,j2);
...
int main(void){
    ...
    char fpot_name[] = "GBumpsData/GaussianBumps_beta10.txt";
    double dt = TAU;
...
// Compute data for constructing the bicubic spline
	...
	// extend the domain
	double extend_fac = 10.0; // used to be 5.0
	*val1_min -= extend_fac*h1; 
	*val1_max += extend_fac*h1;
	*val2_min -= extend_fac*h2;
	*val2_max += extend_fac*h2;
...
// Compute coefficient matrices for the bicubic spline	
	...
	char fname[100];	
	sprintf(fname,"Data/LJ8bins_beta%.0f.txt",BETA);
	fpot = fopen(fname,"w");
	for( i=0; i<N1; i++ ) {
		for( j=0; j<N2; j++ ) {
			ind = i + j*N1;
	...
	sprintf(fname,"Data/LJ8free_energy_beta%.0f.txt",BETA);
	fpot = fopen(fname,"w");
	for( j=0; j<N2; j++ ) {
		for( i=0; i<N1; i++ ) {
			ind = i + j*N1;
	...
	// smooth free energy
	sprintf(fname,"Data/LJ8free_energy_bicubic_matrix_beta%.0f.txt",BETA);
	fpot = fopen(fname,"w");
