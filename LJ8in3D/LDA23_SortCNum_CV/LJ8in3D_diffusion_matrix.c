...
// Compile command:  gcc -Wall LJ8in3D_diffusion_matrix.c timestep_helpers.c LDA_CoordNum_CV.c -lm -O3
...
void total_pot_and_grad(double *conf,int Natoms,double *pot,double *grad,
	double *cvval,double *cvgrad,
	double CV1star,double CV2star,double Bkappa,int CVdim,double *LDAmatrix);
...
// 	double *CVgrad,*CVval;
	...
	CV1diff = CVval[0] - CV1star;
	CV2diff = CVval[1] - CV2star;
	for( j=0; j<dim; j++ ) {
...
// 	double *CV1,*CV2,*CV1grad,*CV2grad;
	...
	total_pot_and_grad(conf0,Natoms,Vpot0,Vgrad0,cvval,cvgrad,
		CV1val,CV2val,Bkappa,CVdim,LDAmatrix);
...
// 	test_CVgrad(conf0);
	...
	for( j = 0; j < Nsteps; j++ ) {		
		...
		total_pot_and_grad(conf1,Natoms,Vpot1,Vgrad1,cvval,cvgrad,
			CV1val,CV2val,Bkappa,CVdim,LDAmatrix);
...
int main(void){
    ...
    double dt = TAU_DiffMatr;
	char *bins;
	char fname[100];
	char fLDAname[] = "Data/LDAbasis.txt";
	int CVdim = CVDIM;
	double *LDAmatrix;

	LDAmatrix = (double *)malloc(Natoms*CVdim*sizeof(double));
	readLDAmatrix(fLDAname,Natoms,CVdim,LDAmatrix);
	...
	// Compute the diffusion matrix at all nonempty bin centers
 	...
 	fconf = fopen("Data/LJ8bins_confs.txt","r");
		...
		for( j=0; j<N2; j++ ) {
			...
			if( confrad > TOL ) {
				...
				diffusion_matrix(Natoms,conf0,dt,Nsteps,grid_CV1[i],grid_CV2[j],
					ind,M11,M12,M22,CVdim,LDAmatrix);
			}
		}
	}
	fclose(fconf);
	...
	sprintf(fname,"Data/LJ8_M11_beta%.0f.txt",BETA);
	fM11 = fopen(fname,"w");
	sprintf(fname,"Data/LJ8_M12_beta%.0f.txt",BETA);
	fM12 = fopen(fname,"w");
	sprintf(fname,"Data/LJ8_M22_beta%.0f.txt",BETA);
	fM22 = fopen(fname,"w");
	...
	sprintf(fname,"Data/LJ8_M11_bicubic_matrix_beta%.0f.txt",BETA);
	fM11 = fopen(fname,"w");
	sprintf(fname,"Data/LJ8_M12_bicubic_matrix_beta%.0f.txt",BETA);
	fM12 = fopen(fname,"w");
	sprintf(fname,"Data/LJ8_M22_bicubic_matrix_beta%.0f.txt",BETA);
	fM22 = fopen(fname,"w");
