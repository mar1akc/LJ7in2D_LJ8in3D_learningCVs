...
// Compile command:  gcc -Wall FFS.c CommittorRC.c LDA_CoordNum_CV.c timestep_helpers.c -lm -O3
...
double exit_flux(double *init_conf,double *exit_conf,struct NNRC *nnrc,double *LDAmatrix,
				 double dt,double *milestones, int Nmilestones);
double prob_next_milestone(double *init_conf,double *exit_conf,struct NNRC *nnrc,double *LDAmatrix,
...
// 			(int)round(BETA),(int)round(BETA)); 
	...
	sprintf(fname,"Data/FFS_BETA%i.txt",(int)round(BETA));
// 	CVval = (double *)malloc(dim4*sizeof(double));
	
	// uniform milestones
	milestones[0] = MSTONE_MIN_VAL;
	milestones[Nmilestones-1] = MSTONE_MAX_VAL;
	// milestones for BETA 20
// 	milestones[1] = MSTONE_MIN_VAL*10; // 1e-4
// 	milestones[2] = MSTONE_MIN_VAL*100; // 1e-3
// 	milestones[3] = MSTONE_MIN_VAL*1000; // 1e-2
// 	mstone_step = (MSTONE_MAX_VAL-milestones[3])/(N_MSTONES - 4);
...
// 	}
	...
	for( k = 0; k < N_REPEAT; k++ ) {
		printf("Run %i\n", k);
		fprintf(fid,"Run %i: from A to B\n", k);
		CPUbegin=clock(); // start time measurement
		
		init_conf(conf0,1); // initial minimum is minimum 1
		eflux_and_pvector[0] = exit_flux(conf0,conf_data1,nnrc,LDAmatrix,dt,milestones,Nmilestones);
		fprintf(fid,"0\t%.4e\n",eflux_and_pvector[0]);
	//  	for( k = 0; k < NEXIT; k++ ) {
	//  		MargotCV(CVval,conf_data1 + k*dim,NATOMS,A1,A2,A3,b1,b2,b3,dim1,dim2,dim3,dim4);
	// 		RCval = reaction_coordinate(CVval,nnrc); 				
	//  		printf("k = %i, CV0 = %.4e, CV1 = %.4e, RCval = %.4e\n",k,CVval[0],CVval[1],RCval);
	//  	}								
		...
		for( mstone_idx = 1; mstone_idx < Nmilestones; mstone_idx++ ) {
			if( mstone_idx%2 == 1 ) {
				...
				eflux_and_pvector[mstone_idx] = prob_next_milestone(conf_data1,conf_data2,
												nnrc,LDAmatrix,dt,milestones,mstone_idx);
	//  			for( k = 0; k < NEXIT; k++ ) {
	//  				MargotCV(CVval,conf_data2 + k*dim,NATOMS,A1,A2,A3,b1,b2,b3,dim1,dim2,dim3,dim4);
	// 				RCval = reaction_coordinate(CVval,nnrc); 				
	//  				printf("k = %i, CV0 = %.4e, CV1 = %.4e, RCval = %.4e\n",k,CVval[0],CVval[1],RCval);
	//  			}								
			}
