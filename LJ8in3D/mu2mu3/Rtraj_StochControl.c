...
// Compile command:  gcc -Wall Rtraj_StochControl.c  CommittorRC.c CV_helpers.c timestep_helpers.c -lm -O3
...
// 	double *bicubic_matrix;
	...
	char fRCname[100];
	sprintf(fRCname,"FEMdataBETA%i/Committor_mu2mu3_BETA%i/RC_NNdata.txt",
			(int)round(BETA),(int)round(BETA)); 
	char fRCname_dim[100]; 
	sprintf(fRCname_dim,"FEMdataBETA%i/Committor_mu2mu3_BETA%i/RC_dimensions.txt",
			(int)round(BETA),(int)round(BETA)); 
	struct NNRC *nnrc;	
