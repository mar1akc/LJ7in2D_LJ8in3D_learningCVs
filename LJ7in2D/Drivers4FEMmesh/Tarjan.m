function SCCcount = Tarjan(V,E,dirname)
global SCCcount
...
for j = 1 : nV
    if index(j) == 0
        [index,lowlink,icurrent,stack,instack] = strongconnect(V(j),icurrent,E,index,lowlink,stack,instack,dirname);
    end
end
...
%% 
function [index,lowlink,icurrent,stack,instack] = strongconnect(v,icurrent,E,index,lowlink,stack,instack,dirname)
global SCCcount
...
if lowlink(v) == index(v)
    ...
    fname = strcat(dirname,sprintf('/LJ7_SCC%d.mat',SCCcount));
    save(fname,'SCC');
end
end
