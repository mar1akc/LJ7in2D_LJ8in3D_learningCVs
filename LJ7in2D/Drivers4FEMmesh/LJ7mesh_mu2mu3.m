function LJ7mesh_mu2mu3()
close all
BETA = 9;
dirname = "/Users/mariacameron/Dropbox/Work/My_Programs/LJ7in2D/";
subdirname = sprintf("mu2mu3/FEMdataBETA%i/",BETA);
dirname = strcat(dirname,subdirname);
pA = readmatrix(strcat(dirname,'Abdry.csv'));
NpA = length(pA);
NA = NpA/2;
pA = (reshape(pA',[2,NA]))';
pA(end,:) = [];
%
pB = readmatrix(strcat(dirname,'Bbdry.csv'));
NpB = length(pB);
NB = NpB/2;
pB = (reshape(pB',[2,NB]))';
pB(end,:) = [];
...
%
p0 = readmatrix(strcat(dirname,'Obdry.csv'));
Np0 = length(p0);
N0 = Np0/2;
...
writematrix(pts,strcat(dirname,'/LJ7_pts.csv'))
writematrix(tr-ones(size(tr)),strcat(dirname,'/LJ7_tri.csv'))
fname_writeA = strcat(dirname,'/LJ7_Abdry.csv');
fname_writeB = strcat(dirname,'/LJ7_Bbdry.csv');
for j = 1 : 2
...
triplot(tr,pts(:,1),pts(:,2));
writematrix(pts,strcat(dirname,'/LJ7_ptsA.csv'));
writematrix(tr-ones(size(tr)),strcat(dirname,'/LJ7_triA.csv'))
...
% [pts,tr] = smoothmesh(pts,tr);
triplot(tr,pts(:,1),pts(:,2));
writematrix(pts,strcat(dirname,'/LJ7_ptsB.csv'));
writematrix(tr-ones(size(tr)),strcat(dirname,'/LJ7_triB.csv'));
