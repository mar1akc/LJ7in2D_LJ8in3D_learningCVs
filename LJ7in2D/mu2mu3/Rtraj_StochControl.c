...
// If BETA == 5, include CommittorRC.h
// If BETA == 7, include CommittorRC_ReLU4.h

// Compile command:  gcc -Wall Rtraj_StochControl.c  CommittorRC_ReLU4.c CV_helpers.c timestep_helpers.c -lm -O3
...
// char PRINT = 'n';
int main(void);
void exit_confs(double *init_conf,double *exit_conf,struct NNRC *nnrc,double dt);
void generate_reactive_trajectories(double *init_conf,double *exit_conf,
					struct NNRC *nnrc,double dt,double *crossover_time,
...
//------------------------------------
void controlled_pot_and_grad(double *conf,struct NNRC *nnrc,
...
// 	void reaction_coordinate_grad(double *RCval,double *RCgrad,double *CV,struct NNRC *nnrc);
	mu2mu3grad(conf,CVval,CVgrad,CVdim);
...
// 	RCval = committor(CVval,nnrc); 
	controlled_pot_and_grad(conf0,nnrc,Vpot0,Vgrad0,CVval,q);
	CVsave[0] = CVval[0];
	CVsave[1] = CVval[1];
	qsave = *q;	
	// start simulation
	...
	while( crossover_count < NEXIT ) {		
		...
		// evaluate the potential and the gradient at the proposed point
// 		total_pot_and_grad(conf1,NATOMS,Vpot1,Vgrad1);
		controlled_pot_and_grad(conf1,nnrc,Vpot1,Vgrad1,CVval,q);
		ch = MALAstep(conf0,conf1,NATOMS,dt,Vpot0,Vpot1,Vgrad1,w);
...
// 	double *bicubic_matrix;
	...
	char fRCname[100];
	sprintf(fRCname,"Committor_mu2mu3_BETA%i/RC_NNdata.txt",
			(int)round(BETA)); 
	char fRCname_dim[100]; 
	sprintf(fRCname_dim,"Committor_mu2mu3_BETA%i/RC_dimensions.txt",
			(int)round(BETA)); 
	struct NNRC *nnrc;	
	...
	if( fpar == NULL ) {
		printf("Cannot find the file Data/bicubic_params.txt\n");
		exit(1);
	}
	fscanf(fpar,"%i\n",&N1);
	fscanf(fpar,"%i\n",&N2);
	...
	sprintf(fname,"Data/RtrajStochControl/Rtraj_CrossoverTime_BETA%i.txt",(int)round(BETA));	
	...
	exit_confs(conf0,A_conf,nnrc,dt);
	generate_reactive_trajectories(A_conf,B_conf,nnrc,dt,crossover_time,
