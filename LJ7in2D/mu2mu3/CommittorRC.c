...
// compile command: gcc -Wall CommittorRC.c CV_helpers.c timestep_helpers.c -lm -O3
...
double tanh_grad(double z) { // tanh'(z) = 1 - tanh^2(z)
	double aux = tanh(z);
	return 1.0 - aux*aux;
}
//-------------------------------------------------------------
...
// 	RCval = 1.0 - RCval; // flip sign because the committor is from hexagon to trapezoid
...
void reaction_coordinate_grad(double *RCval,double *RCgrad,double *CV,struct NNRC *nnrc) {
	...
	for( j = 0; j < dim1; j++ ) {
		D1[j] = tanh_grad(w1[j]); // D1 = diag(dtanh(w1))
		w1[j] = tanh(w1[j]);   // w1 = tanh(w1)
	}
	...
	for( j = 0; j < dim2; j++ ) {
		D2[j] = tanh_grad(w2[j]); // D2 = diag(dtanh(w2))
		w2[j] = tanh(w2[j]);  // w2 = tanh(w2)
	}
...
// 	// flip sign because the committor is from hexagon to trapezoid, 
// 	// while we need from trapezoid to hexagon
// 	RCval = 1.0 - RCval; 
// 	for( j = 0; j < dim0; j++ ) {
// 		RCgrad[j] = -RCgrad[j];   // RC_grad = D3*J3
// 	}
...
// 	char fRCname[] = "FEMdataBETA10/Committor_mu2mu3_3hidden_BETA10/RC_NNdata.txt";
// 	char fRCname_dim[] = "FEMdataBETA10/Committor_mu2mu3_3hidden_BETA10/RC_dimensions.txt";
// 	struct NNRC *nnrc;	
