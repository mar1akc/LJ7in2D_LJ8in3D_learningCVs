...
// Compile command:  gcc -Wall LJ7in2D_WTMetad_mu2mu3.c CV_helpers.c timestep_helpers.c -lm -O3
...
// 	for( k = 0; k < dim; k++ ) printf("k = %i, conf0 = %.6e\n",k,conf0[k]);
	...
	sprintf(fbump_name,"GbumpsData/GaussianBumps_new_beta%.f.txt",BETA); 
