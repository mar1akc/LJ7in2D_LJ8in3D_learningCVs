...
// Compile command:  gcc -Wall brute_force.c CommittorRC_ReLU4.c CV_helpers.c timestep_helpers.c -lm -O3
...
#define N_REPEAT 10 // the number of brute force runs
#define INFTY 1.0e6

void run_trajectory(FILE *fid,int Nsteps,int Natoms,double *conf0,double dt,//double t_start,
	struct NNRC *nnrc,int *AB_event_counter,double *TA,double *TB,double *TlastA,double *TlastB);
...
// 		printf("In run_trajectory, j = %i, RCval = %.4e\n",j,RCval);	
	...
	printf("Done! Beta = %.1f\n",BETA);
	printf("TinA(j) = %.4e; TinB(j) = %.4e; T_OmegaAB(j) = %.4e; rateAB = %.4e(j);\n",
...
//     char fname[100];
	...
	for( n = 0; n < N_REPEAT; n++ ) {
		...
		fprintf(fid,"Beta = %.1f, Run # %i\n",BETA,n);
 		run_trajectory(fid,Nsteps,NATOMS,conf0,dt,nnrc,AB_event_counter+n,TinA+n,TinB+n,TlastA+n,TlastB+n);
