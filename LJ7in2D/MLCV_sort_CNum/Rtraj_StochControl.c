...
// Compile command:  gcc -Wall Rtraj_StochControl.c  CommittorRC.c MargotColVar_CoordNum.c timestep_helpers.c -lm -O3
...
// char PRINT = 'n';
int main(void);
void exit_confs(double *init_conf,double *exit_conf,struct NNRC *nnrc,struct NN *nncv,double dt);
void generate_reactive_trajectories(double *init_conf,double *exit_conf,
					struct NNRC *nnrc,struct NN *nncv,double dt,double *crossover_time,
...
void controlled_pot_and_grad(double *conf,struct NNRC *nnrc,struct NN *nncv,
...
void exit_confs(double *init_conf,double *exit_conf,struct NNRC *nnrc,struct NN *nncv,double dt) {
	...
	int j,k,dim = NATOMS*DIM,CVdim = CVDIM;
	double *gauss01;
	int kshift;
	...
	printf("step = %li, time = %.4e, exit_count = %i\n",step,dt*step,exit_count);
...
// 	RCval = committor(CVval,nnrc); 
	controlled_pot_and_grad(conf0,nnrc,nncv,Vpot0,Vgrad0,CVval,q);
	CVsave[0] = CVval[0];
	CVsave[1] = CVval[1];
	qsave = *q;	
	// start simulation
	...
	while( crossover_count < NEXIT ) {		
		...
		// evaluate the potential and the gradient at the proposed point
// 		total_pot_and_grad(conf1,NATOMS,Vpot1,Vgrad1);
		controlled_pot_and_grad(conf1,nnrc,nncv,Vpot1,Vgrad1,CVval,q);
		ch = MALAstep(conf0,conf1,NATOMS,dt,Vpot0,Vpot1,Vgrad1,w);
...
// 	double *bicubic_matrix;
	...
	char fRCname[100];
	sprintf(fRCname,"FEMdataBETA%i/Committor_CV_SortCNum_BETA%i/RC_NNdata.txt",
			(int)round(BETA),(int)round(BETA)); 
	char fRCname_dim[100]; 
	sprintf(fRCname_dim,"FEMdataBETA%i/Committor_CV_SortCNum_BETA%i/RC_dimensions.txt",
			(int)round(BETA),(int)round(BETA)); 
	struct NNRC *nnrc;	
	...
	sprintf(fname,"Data/RtrajStochControl/Rtraj_CrossoverTime_BETA%i_run%i.txt",(int)round(BETA),run_id);	
	...
	exit_confs(conf0,A_conf,nnrc,nncv,dt);
	generate_reactive_trajectories(A_conf,B_conf,nnrc,nncv,dt,crossover_time,
	...
	fprintf(fid,"mean crossover time = %.6e\n",mean_crossover_time); 
	double aux;	
	for( j = 0; j< NEXIT; j++ ) {
	...
	fprintf(fid,"std crossover time = %.6e\n",std_crossover_time); 
	...
	char fname1[100];
	int ind;
	sprintf(fname1,"Data/RtrajStochControl/Rtraj_bins_beta%.i.txt",(int)round(BETA));
	FILE *fpot;
	fpot = fopen(fname1,"w");
	for( i=0; i<N1; i++ ) {
		for( j=0; j<N2; j++ ) {
