...
// Compile command:  gcc -Wall brute_force.c CommittorRC.c MargotColVar_CoordNum.c timestep_helpers.c -lm -O3
...
void run_trajectory(FILE *fid,int Nsteps,int Natoms,double *conf0,double dt,struct NN *nncv,
...
// 	srandom(time(NULL));
	...
	for( n = 0; n < N_REPEAT; n++ ) {
		...
		fprintf(fid,"Beta = %.1f, Run # %i\n",BETA,n);
 		run_trajectory(fid,Nsteps,NATOMS,conf0,dt,nncv,nnrc,AB_event_counter+n,TinA+n,TinB+n,TlastA+n,TlastB+n);
