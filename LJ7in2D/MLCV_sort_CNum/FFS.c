...
// Compile command:  gcc -Wall FFS.c CommittorRC.c MargotColVar_CoordNum.c timestep_helpers.c -lm -O3
...
// 			if( RCval > 0.96) printf("RCval = %.4e, last_milestone = %.4e\n",RCval,last_milestone);
...
// 					printf("j = %i, conf0 = %.4e\n",j,conf0[j]);
				...
				RCval = reaction_coordinate(CVval,nnrc); 	
				RCprev = RCval;
// 				printf("Reset: CVdim = %i, CV = [%.4e,%.4e], RCval = %.4e\n",CVdim,CVval[0],CVval[1],RCval);
			}
		}
		step++;
	}
...
// 	int run_id = 1;
	...
	for( k = 0; k < N_MINIMA; k++ ) {
		...
		printf("MIN %i: MargotCV [%.4e,%.4e], RCval = %.4e\n",k,CV[0],CV[1],RCval);
// 		for( j = 0; j < dim; j++ ) printf("j = %i, conf0 = %.4e\n",j,conf0[j]);
	}
//    	exit(1);
